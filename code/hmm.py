...
#         print(self.tagDict[0])
#             # torch.full(len(self.tagset), dtype=torch.long)
#             # tensor = torch.full((12,), 1e-45)
...
#         # if not self.vi_prepared:
#         #     self.vi_prepared = self.prepare_viterbi_tagging(corpus)
...
# HMM-awesome tagger
###
class AwesomeHMM (HiddenMarkovModel):
    def __init__(self, tagset, vocab, lexicon, unigram=False):
        super().__init__(tagset, vocab, lexicon, unigram)
        print("this is HMM Awesome initialization")
        self.tagDict = {}
        for v in self.vocab:
            self.tagDict[self.integerize_word(v)] = torch.full((len(self.tagset,),), 1e-45)
        print("self.tagDict")
    ...
    def train(self,
        ...
        @classmethod
        def load(cls, model_path: Path, gpu: bool = False) -> AwesomeHMM:
            logger.info(f"Loading model from {model_path}")
            model = torch.load(model_path, map_location=('cuda' if gpu else 'cpu'))
            if not isinstance(model, cls):
                raise ValueError(f"Type Error: expected object of type {cls} but got {type(model)} from saved file {model_path}.")
