...
class HiddenMarkovModel(nn.Module):
    ...
    def init_params(self) -> None:
        ...
        WA = 0.01*torch.rand(1 if self.unigram # just one row if unigram model
                             ...
                             self.k, self.k)           # one column per tag t
    ...
    def updateAB(self) -> None:
        ...
        A = F.softmax(self._WA, dim=2)       # run softmax on params to get transition distributions
    ...
    def printAB(self) -> None:
        ...
        for s in range(self.A.size(0)):   # rows
            for u in range(self.A.size(1)):
                row = [str(self.tagset[s])] + [f"{self.A[s,t,u]:.3f}" for t in range(self.A.size(1))]
                print("\t".join(row))
        print("\nEmission matrix B:")        
    ...
    def log_forward(self, sentence: Sentence, corpus: TaggedCorpus) -> TorchScalar:
        ...
        if len(nan_indices) > 0:
            print(f"NaN values in matrix A at indices:")
            for index in nan_indices:
                print(f"Index {index}: {self.A[index]}")
        else:
            print(f"No NaN values in matrix A")
        ...
        for j in range(1, n+1):
            ...
            if tag is None:
                A_contrib = torch.log(self.A[:, :, :])
                alpha[j, :] = logsumexp_new(
                    alpha[j - 1, :] + A_contrib + torch.log(self.B[:, word]),
                    dim=0, keepdim=False, safe_inf=True
                )
            else:
                A_contrib = torch.log(self.A[:, :, tag])
                alpha[j, :] = logsumexp_new(
                    alpha[j - 1, :] + A_contrib + torch.log(self.B[tag, word]),
                    dim=0, keepdim=False, safe_inf=True
                )
                print("here")
        ...
        alpha[n+1, self.eos_t] = logsumexp_new(
            alpha[n, :] + torch.log(self.A[:, :, self.eos_t].unsqueeze(0)),
            dim=0, keepdim=False, safe_inf=True
        )
